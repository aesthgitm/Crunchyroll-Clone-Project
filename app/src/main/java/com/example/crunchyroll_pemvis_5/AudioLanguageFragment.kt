...
class AudioLanguageFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_audio_language, container, false)

        val btnBack = view.findViewById<View>(R.id.btn_back_audio_lang)
        val containerOptions = view.findViewById<LinearLayout>(R.id.container_audio_options)
        ...
        for (lang in languages) {
            ...
            val radioButton = RadioButton(context).apply {
                ...
                isChecked = (MockData.audioLanguage == lang)
                isClickable = false
                isFocusable = false
            }
            ...
            row.setOnClickListener {
                MockData.audioLanguage = lang
                val userId = com.google.firebase.auth.FirebaseAuth.getInstance().currentUser?.uid ?: ""
                if (userId.isNotEmpty()) {
                    FirestoreHelper().saveCurrentUserSnapshot(userId) { success ->
                        activity?.runOnUiThread {
                            val msg = if (success) {
                                "Bahasa audio diatur ke: $lang"
                            } else {
                                "Bahasa audio diperbarui lokal, gagal sinkron cloud"
                            }
                    ...
                    Toast.makeText(context, "Bahasa audio diatur ke: $lang", Toast.LENGTH_SHORT).show()
                    activity?.supportFragmentManager?.popBackStack()
                }
            }
