...
class DaftarSayaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_daftar_saya, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(tabLayout, viewPager) { tab, position ->
            ...
            context?.let { ctx ->
                when (position) {
                    0 -> {
                        tabText.text = "Daftar Tonton"
                        tabText.setTextColor(ContextCompat.getColor(ctx, R.color.white))
                    }
                    1 -> {
                        tabText.text = "Crunchylist"
                    ...
                    3 -> {
                        tabText.text = "Unduhan"
                        tabText.setTextColor(ContextCompat.getColor(ctx, R.color.tab_unselected_grey))
                        tabIcon.visibility = View.VISIBLE
                        tabIcon.imageTintList = ColorStateList.valueOf(ContextCompat.getColor(ctx, R.color.crunchyroll_gold))
                    }
                }
            }
